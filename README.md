...
contract F1PaddockClub is AccessControlEnumerable, ReentrancyGuard {
    ...
    function getCurrentOwner(uint256 _ticketId) public view returns (address) {
        return tickets[_ticketId].currentOwner;
    }

    function getQRCodeHash(uint256 _ticketId) public view returns (string memory) {
        return tickets[_ticketId].qrCodeHash;
    }

    function verifyOwnership(uint256 _ticketId, address _owner) public view returns (bool) {
        return tickets[_ticketId].currentOwner == _owner;
    }
